...
The program in [examples/basic](examples/basic) builds a pipeline from five
small steps. Each step has its own input type and its own output type, and
`Then` and `Par` change the type of the chain at each link.

[embedmd]:# (examples/basic/main.go go)
```go
package main
...
Run it with `cd examples/basic && go run .`.
